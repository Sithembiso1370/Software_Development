    ...
    function get_total_joined_rows($connection,$fromparam,$toparam,$openclosedparam,$tableparam) 
    {
        ...
        if($fromparam == "" || $toparam == ""){
            ...
            if($openclosedparam == 'open'){
                // OPEN
                $query = "
                SELECT * FROM ". $tableparam."  WHERE `s_qoute_reference` NOT IN (SELECT `s_qoute_reference` FROM customer_invoices) ORDER BY `submitted_on` DESC LIMIT 0,17 
                ";
            ...
            elseif($openclosedparam == 'closed')
            {
                ...
                // CLOSED
                $query = "
                SELECT * FROM ". $tableparam."  WHERE `s_qoute_reference` IN (SELECT `s_qoute_reference` FROM customer_invoices) ORDER BY `submitted_on` DESC  LIMIT 0,17 
                ";
                $statement = $connection->prepare($query);
                $statement->execute();
            ...
            if($openclosedparam == 'open'){
                ...
                AND  `s_qoute_reference` NOT IN (SELECT `s_qoute_reference` FROM customer_invoices) ORDER BY `submitted_on` DESC  LIMIT 0,17
                ";
                $statement = $connection->prepare($query);
                $statement->execute();
            }
            elseif($openclosedparam == 'closed')
            {
                ...
                $query = "
                SELECT * FROM ". $tableparam."  WHERE `submitted_on` >= '".$fromparam."' AND `submitted_on` <= '".$toparam."' 
                AND  `s_qoute_reference` IN (SELECT `s_qoute_reference` FROM customer_invoices) ORDER BY `submitted_on` DESC  LIMIT 0,17
                ";
                $statement = $connection->prepare($query);
                $statement->execute();
