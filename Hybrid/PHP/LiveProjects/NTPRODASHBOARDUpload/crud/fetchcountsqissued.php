...
function get_count_joined_rows($connection,$tableparam,$openclosedparam,$fromparam,$toparam)
{
    if($fromparam == "" || $toparam == ""){
        if($openclosedparam == 'open'){
            // OPEN
            $query = "
            SELECT * FROM ". $tableparam." WHERE `s_qoute_reference` NOT IN (SELECT `s_qoute_reference` FROM customer_invoices2)
            ";
        ...
        elseif($openclosedparam == 'closed')
        {
            ...
            // CLOSED
            $query = "
            SELECT * FROM ". $tableparam." WHERE `s_qoute_reference`  IN (SELECT `s_qoute_reference` FROM customer_invoices2)
            ";
            $statement = $connection->prepare($query);
            $statement->execute();
        ...
        if($openclosedparam == 'open'){
            ...
            AND  `s_qoute_reference` NOT IN (SELECT `s_qoute_reference` FROM customer_invoices2)  
            ";
            $statement = $connection->prepare($query);
            $statement->execute();
        }
